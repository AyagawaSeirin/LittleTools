...
<template>
  <ToolPageHeader title="时间戳转换" description="Unix 时间戳与日期时间双向转换，支持秒和毫秒" :icon="ClockCircleOutlined">
  ...
  <ToolCard title="时间戳转日期" description="自动模式会根据位数判断秒级或毫秒级。">
    <div class="timestamp-input">
      ...
      <a-select v-model:value="inputUnit"><a-select-option value="auto">自动判断</a-select-option><a-select-option value="s">秒</a-select-option><a-select-option value="ms">毫秒</a-select-option></a-select>
    ...
    <div v-else class="time-results">
      <div><span>本地时间 · {{ localZone }}</span><code>{{ timestampResult.local }}</code><a-button type="text" size="small" aria-label="复制本地时间" @click="copy(timestampResult.local)"><CopyOutlined /></a-button></div>
      <div><span>UTC 时间</span><code>{{ timestampResult.utc }}</code><a-button type="text" size="small" aria-label="复制 UTC 时间" @click="copy(timestampResult.utc)"><CopyOutlined /></a-button></div>
      <div><span>ISO 8601</span><code>{{ timestampResult.iso }}</code><a-button type="text" size="small" aria-label="复制 ISO 时间" @click="copy(timestampResult.iso)"><CopyOutlined /></a-button></div>
    </div>
  </ToolCard>

  <ToolCard title="日期转时间戳">
    <div class="date-to-timestamp">
      <div class="form-field"><span class="field-label">本地日期与时间</span><a-date-picker v-model:value="selectedDate" :allow-clear="false" show-time format="YYYY-MM-DD HH:mm:ss" style="width: 100%" /></div>
      ...
      <div class="timestamp-output"><span>{{ outputUnit === 's' ? '秒级时间戳' : '毫秒级时间戳' }}</span><code>{{ dateTimestamp }}</code><a-button type="text" aria-label="复制时间戳" @click="copy(dateTimestamp)"><CopyOutlined /></a-button></div>
    </div>
  </ToolCard>
</template>

<style scoped>
.now-chip { display: flex; flex-direction: column; align-items: flex-end; }
.now-chip span { color: var(--text-muted); font-size: 12px; }
.now-chip b { margin-top: 2px; font-family: monospace; font-size: 14px; }
.timestamp-input { display: grid; grid-template-columns: minmax(0, 1fr) 120px auto auto; gap: 10px; }
...
.date-to-timestamp { display: grid; grid-template-columns: minmax(0, 1fr) 160px; gap: 18px; }
...
.timestamp-output code { min-width: 0; overflow-wrap: anywhere; color: var(--accent-text); font-size: 18px; font-weight: 600; }
@container (max-width: 620px) { .timestamp-input { grid-template-columns: minmax(0, 1fr) 120px; } .timestamp-input .ant-btn { width: 100%; } .time-results > div { grid-template-columns: minmax(0, 1fr) auto; padding-top: 10px; padding-bottom: 10px; } .time-results code { grid-column: 1; margin-top: 5px; } .time-results .ant-btn { grid-column: 2; grid-row: 1 / 3; } .date-to-timestamp { grid-template-columns: minmax(0, 1fr); } .timestamp-output { grid-column: auto; grid-template-columns: minmax(0, 1fr) auto; } .timestamp-output code { grid-column: 1; margin-top: 5px; } .timestamp-output .ant-btn { grid-column: 2; grid-row: 1 / 3; } }
@container (max-width: 700px) { .now-chip { align-items: flex-start; } }
@container (max-width: 360px) { .timestamp-input { grid-template-columns: minmax(0, 1fr); } }
</style>
